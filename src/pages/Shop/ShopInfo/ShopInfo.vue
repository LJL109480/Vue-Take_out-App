  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item" :class="supportsClass[support.type]" v-for="(support, index) in info.supports" :key="index">
...
<script>
  ...
  export default{
    ...
    mounted(){
      //初始显示商家信息时，info内容还未请求回来，直接调用info回报错，
      //判断info内是否有数据，有显示，无return
      if(!this.info.name){
        return
      }
      this._initScroll()
    ...
    methods:{
      _initScroll(){
        ...
        const width = (listWidth+space) * count - space + 'px'
    ...
    watch:{
      //监视info变化，当在商家页面刷新时，info请求回来，则正常执行滑动
      info(){
        this.$nextTick(()=>{
          this._initScroll()
        })
